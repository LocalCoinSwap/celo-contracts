...
const tradeID = "0x8a221ffd05e94a16b4590b508d085ef6";
...
const value = "10000000000000000"; // 0.01 cUSD
const fee = 100;
...
const printBalances = async () => {
  ...
  const relayerBalance = await stabletoken.balanceOf(relayer.address);
  const sellerBalance = await stabletoken.balanceOf(seller.address);
  const buyerBalance = await stabletoken.balanceOf(buyer.address);
  const contractBalance = await stabletoken.balanceOf(CONTRACT_ADDRESS);

  console.log(`RELAYER: ${relayer.address} cUSD: ${relayerBalance.toString()}`);
  console.log(`SELLER:  ${seller.address} cUSD: ${sellerBalance.toString()}`);
  console.log(`BUYER:   ${buyer.address} cUSD: ${buyerBalance.toString()}`);
  console.log(
    `CONTRACT:${CONTRACT_ADDRESS} cUSD: ${contractBalance.toString()}`
  );
};
...
const fundEscrow = async () => {
  ...
  console.log("CUSD token address", stabletoken.address);

  const approveTx = await stabletoken.approve(CONTRACT_ADDRESS, value).send({
    from: seller.address,
  });
  const approveReceipt = await approveTx.waitReceipt();
  console.log(approveReceipt);

  const allow = await stabletoken.allowance(seller.address, CONTRACT_ADDRESS);
...
const happyCeloTrade = async () => {
  await printBalances();
  await fundEscrow();
  await printBalances();
  await checkEscrow();
  await releaseEscrowToBuyer();
  await printBalances();
};

happyCeloTrade().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});

export default happyCeloTrade;
