...
const tradeID = "0x8a221ffd05e94a16b4590b508d085ef9";
...
const revertToSeller = async () => {
  ...
  console.log("Building dispute resolve txn");
  const txObject = await instance.methods.resolveDispute(
    tradeID,
    seller.address,
    buyer.address,
    value,
  ...
  console.log("Sending dispute resolve txn");
  const tx = await kit.sendTransactionObject(txObject, {
...
const celoGoldSellerWinsDispute = async () => {
  await printBalances();
  await fundEscrow();
  await printBalances();
  await checkEscrow();
  await revertToSeller();
  await printBalances();
};

celoGoldSellerWinsDispute().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});

export default celoGoldSellerWinsDispute;
